...
export class PrReviewProcessor extends WorkerHost {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    ...
    this.eventBroadcaster.broadcastJobUpdate(
      analysisJobId,
      'PROCESSING',
      'FETCHING_DIFF',
      undefined,
    ...
    try {
      ...
      if (!diff || diff.trim().length === 0) {
        ...
        if (checkRunId) {
          await this.githubCheckRunService
            .completeCheckRun(
              installationId,
              owner,
              repoName,
              checkRunId,
              'success',
              'MergeMind Code Review',
              'No diff content found or diff is empty.',
            )
            .catch((e: any) =>
              this.logger.warn(`Failed to complete check run: ${e.message}`),
            );
        }
        ...
        if (checkRunId) {
          await this.githubCheckRunService
            .completeCheckRun(
              ...
              'success',
              'MergeMind Code Review',
              'Empty clean diff after pre-processing.',
            )
            .catch((e: any) =>
              this.logger.warn(`Failed to complete check run: ${e.message}`),
            );
