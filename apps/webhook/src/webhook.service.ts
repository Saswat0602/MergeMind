...
export class WebhookService {
  ...
  async processInstallation(payload: WebhookPayload) {
    if (!payload.installation) {
      return { skipped: true, reason: 'No installation in payload' };
    }

    const { installation, action } = payload;
    const isActive = action !== 'deleted' && action !== 'suspend';
  ...
  async processInstallationRepositories(payload: WebhookPayload) {
    if (!payload.installation) {
      return { skipped: true, reason: 'No installation in payload' };
    }
    ...
    if (!dbInstallation) {
      this.logger.warn(`Installation not found for ID: ${installation.id}`);
      return { skipped: true, reason: 'Installation not found' };
    }
    ...
    if (repositories_removed && repositories_removed.length > 0) {
      for (const repo of repositories_removed) {
        // Soft delete or just remove mapping - for now, just log or mark inactive.
        // If we want to strictly delete:
