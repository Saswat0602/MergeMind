...
Binary files differ`;
    expect(getFilePathFromDiffBlock(block)).toBe('apps/dashboard/public/logo.png');
  });
...
+const y = 2;`;
    ...
    const { filteredDiff, skippedCount, skippedSummary } = filterAndTruncateDiff(diff);
  ...
  it('should filter out files exceeding line limits', () => {
    ...
    const lines = ['diff --git a/src/massive.ts b/src/massive.ts', '--- a/src/massive.ts', '+++ b/src/massive.ts'];
