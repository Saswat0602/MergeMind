...
export function filterAndTruncateDiff(diffContent: string): {
  ...
  for (const block of fileBlocks) {
    ...
    if (blockLines.length > REVIEW_EXCLUSIONS.MAX_DIFF_LINE_COUNT) {
      skippedCount++;
      skippedSummary.push(`${filePath} (too large: ${blockLines.length} lines)`);
      logger.warn(`Skipping file from review (too large - ${blockLines.length} lines): ${filePath}`);
      continue;
    }
  ...
  for (const block of acceptedBlocks) {
    if (totalLength + block.length > REVIEW_EXCLUSIONS.MAX_TOTAL_DIFF_CHARACTERS) {
      budgetExceeded = true;
      ...
      logger.warn(`Omitting file from review (exceeded total character budget): ${filePath}`);
      continue;
    }
